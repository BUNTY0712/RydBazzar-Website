...
// Reusable Autocomplete Input Component adapted for Corridor parameters
const LocationInput = ({ 
  label, 
  value, 
  placeholder, 
  ...
  const [triggerMutation, { data: response, isLoading: isFetching }] = mutationHook();
  ...
  useEffect(() => {
    if (value.trim().length > 2 && isFocused && !disabled) {
      triggerMutation({ ...mutationArgs, queryText: value });
      setIsOpen(true);
    } else {
      setIsOpen(false);
    }
    ...
    <div className="relative" ref={containerRef}>
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          placeholder={disabled ? "Please select pickup location first" : placeholder}
          className={`w-full border rounded-lg py-3 pl-8 pr-10 outline-none text-sm placeholder-gray-400 transition-colors ${
            disabled
      ...
      {isOpen && suggestionsList.length > 0 && (
        <ul className="absolute z-20 w-full bg-white border border-gray-100 rounded-lg mt-1 max-h-60 overflow-y-auto shadow-lg divide-y divide-gray-100">
          {suggestionsList.map((item: any) => {
            const displayName = item.label || item.airportName || item.placeName || item.city || "";
            return (
              <li
                key={item.placeId || item.id || item.orderIndex}
                onClick={() => {
                  setIsFocused(false);
...
const AirportForm = () => {
  ...
  const [getBestPrice, { isLoading: isSubmitting }] = useGetBestPriceWithoutTollMutation();
  ...
  const handleSelectLocation = (key: "pickup" | "drop", item: any) => {
    const name = item.label || item.airportName || item.city || item.placeName || "";
    ...
    if (key === "pickup") {
      ...
      setForm((prev) => ({
        ...prev,
        drop: { address: "", latitude: null, longitude: null }
      }));
    }
  };
  ...
  const useWrappedPickupSuggestions = () => {
    const [trigger, result] = useGetCorridorPickupSuggestionsMutation();
    const wrappedTrigger = ({ queryText }: { queryText: string }) => 
      trigger({ fromQuery: queryText, limit: 10 });
    return [wrappedTrigger, result] as const;
  };

  const useWrappedDropSuggestions = () => {
    const [trigger, result] = useGetCorridorDropSuggestionsMutation();
    const wrappedTrigger = ({ queryText }: { queryText: string }) => {
      if (!selectedPickup) return;
      trigger({
        fromLatitude: selectedPickup.latitude,
        fromLongitude: selectedPickup.longitude,
        fromPlaceId: selectedPickup.placeId || "",
        fromAirportMasterId: selectedPickup.id || "",
        q: queryText,
        limit: 10,
      });
    };
  ...
  return (
    <div className="space-y-4">
      {/* Pickup Search Component */}
      <LocationInput
        label="Airport / Location Pickup"
        ...
        mutationHook={useWrappedPickupSuggestions}
      />

      {/* Drop Search Component */}
      <LocationInput
        label="Airport / Location Dropoff"
        ...
        mutationHook={useWrappedDropSuggestions}
      />
      ...
      <div>
        ...
        <div className="relative flex items-center">
          <select
            ...
            className="w-full border border-gray-200 rounded-lg py-3 px-3.5 pr-10 text-sm text-gray-700 appearance-none bg-transparent outline-none focus:border-gray-400 cursor-pointer bg-white"
          >
      ...
      >
        {isSubmitting && <Loader2 size={18} className="animate-spin text-gray-900" />}
        Find My Airport Cab
      </button>
    </div>
  );
