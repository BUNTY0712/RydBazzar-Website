...
export function AddProductModal({ refetch }: any) {
  ...
  const handleAddGalleryImages = (newFiles: FileList) => {
    ...
    setFormData((prev) => {
      // Filter out files that have the exact same name and size to prevent duplicates
      ...
      return {
        ...prev,
        images: [...prev.images, ...uniqueNewFiles], // Merges old selections + new selections
      };
    });
  };
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b bg-muted/40 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Add new product details</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="Main Thumbnail">
            <Input
              ...
              accept="image/*"
              className="pt-1.5"
              onChange={(e) => handleChange("thumbnail", e.target.files?.[0] || null)}
            />
          </Field>
              ...
              <Input
                ...
                onChange={(e) => {
                  if (e.target.files && e.target.files.length > 0) {
                    ...
                    // Dynamically flush out the target string value so HTML lets you pick images again
                    e.target.value = ""; 
                  }
                }}
              />
              ...
              {formData.images.length > 0 && (
                <div className="mt-3 space-y-2">
                  <p className="text-xs text-cyan-600 font-medium">
                    Staged Gallery Images ({formData.images.length})
                  </p>
                  <div className="flex flex-wrap gap-3 p-3 rounded-xl border border-dashed bg-muted/30">
                    {formData.images.map((file, index) => {
                      ...
                      return (
                        <div 
                          key={`${file.name}-${index}`} 
                          className="group relative h-16 w-16 rounded-lg border bg-background shadow-sm overflow-hidden"
                        >
                          <img
                            src={imgUrl}
                            alt="Gallery Staged Preview"
